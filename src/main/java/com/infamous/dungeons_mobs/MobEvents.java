...
//        }
		if (event.getEntity() instanceof Drowned) {
			...
			((GoalSelectorAccessor) drownedEntity.goalSelector).getAvailableGoals()
					.removeIf(pg -> pg.getPriority() == 2 && pg.getGoal() instanceof RangedAttackGoal);
		...
		if (event.getEntity() instanceof ThrownTrident) {
			((IHasItemStackData) event.getEntity())
					.setDataItem(((TridentEntityAccessor) event.getEntity()).getTridentItem());
		}
	}
		...
		if (livingEntity instanceof Mob && ConvertibleHelper.convertsInWater((Mob) livingEntity)) {
			...
			if (!mob.level.isClientSide && mob.isAlive() && !mob.isNoAi()) {
				...
				if (convertibleCap.isConverting()) {
					...
					if (convertibleCap.getConversionTime() < 0 && net.minecraftforge.event.ForgeEventFactory
							.canLivingConvert(mob, convertToType, convertibleCap::setConversionTime)) {
			...
			if (shooter instanceof IllusionerCloneEntity) {
				...
				if (!arrowEntity.level.isClientSide) {
					...
					for (int i = 0; i < 2; ++i) {
						...
						arrowEntity.level.addParticle(ParticleTypes.POOF, arrowEntity.getRandomX(1.0D),
								arrowEntity.getRandomY(), arrowEntity.getRandomZ(1.0D), d0, d1, d2);
					}
				}
			}
		}
	...
	/*
	 * @SubscribeEvent public static void onProjectileImpact(ProjectileImpactEvent
	 * event){ RayTraceResult rayTraceResult = event.getRayTraceResult();
	 * if(rayTraceResult instanceof EntityRayTraceResult){ EntityRayTraceResult
	...
	private static void handlePillarProtection(ExplosionEvent.Detonate event) {
		...
		while (it.hasNext()) {
			...
			if (notAPillar && currentEntity != null) {
				boolean protectedByPillar = entityProtectedByPillar(currentEntity, potentialProtectingPillars,
						detonationOrigin);
	...
	private static boolean entityProtectedByPillar(Entity entity, List<ConstructEntity> pillarEntities,
			BlockPos detonationOrigin) {
		...
		for (ConstructEntity pillarEntity : pillarEntities) {
			...
			boolean canProtect = distanceEntityToPillar + distanceExplosionToPillar <= distanceExplosionToEntity
					+ widthAllowance;
			if (canProtect)
				return true;
		}
	...
	/*
	 * @SubscribeEvent public static void onMobsSpawn(WorldEvent.PotentialSpawns
	 * event){ IWorld world = event.getWorld(); BlockPos blockPos = event.getPos();
