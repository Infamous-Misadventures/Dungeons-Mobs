...
public class MobEvents {
    ...
    public static void onIllusionerBecomesInvisible(PotionEvent.PotionAddedEvent event){
        if(event.getEntityLiving() instanceof IllusionerEntity){
            IllusionerEntity illusionerEntity = (IllusionerEntity) event.getEntityLiving();
            if(!illusionerEntity.isPotionActive(Effects.INVISIBILITY) && event.getPotionEffect().getPotion() == Effects.INVISIBILITY && !illusionerEntity.world.isRemote && illusionerEntity.isSpellcasting()){
    ...
    private static void summonIllusionerClones(IllusionerEntity illusionerEntity){
        ...
        for(int i = 0; i < mobsToSummon; ++i) {
            ...
            illusionerCloneEntity.onInitialSpawn(illusionerCloneEntity.world, difficultyForLocation, SpawnReason.MOB_SUMMONED, (ILivingEntityData)null, (CompoundNBT)null);
