...
public abstract class AbstractStaffItem extends Item {
	...
	public InteractionResultHolder<ItemStack> use(Level world, Player player, InteractionHand hand) {
		...
		Vec3 rayTraceVector = eyeVector.add(lookVector.x * RAYTRACE_DISTANCE, lookVector.y * RAYTRACE_DISTANCE,
				lookVector.z * RAYTRACE_DISTANCE);
		AABB rayTraceBoundingBox = player.getBoundingBox().expandTowards(lookVector.scale(RAYTRACE_DISTANCE))
				.inflate(1.0D, 1.0D, 1.0D);
		EntityHitResult entityRTR = ProjectileUtil.getEntityHitResult(world, player, eyeVector, rayTraceVector,
				rayTraceBoundingBox,
	...
	protected abstract void activateStaff(Player playerIn, BlockPos targetPos, ItemStack itemStack,
