	...
	public void renderByItem(ItemStack stack, ItemTransforms.TransformType transformType, PoseStack matrixStack,
		...
		if (item instanceof RoyalGuardShieldItem) {
			...
			VertexConsumer ivertexbuilder = rendermaterial.sprite().wrap(ItemRenderer.getFoilBufferDirect(buffer,
					this.royalGuardShieldModel.renderType(rendermaterial.atlasLocation()), true, stack.hasFoil()));
			this.royalGuardShieldModel.handle().render(matrixStack, ivertexbuilder, combinedLight, combinedOverlay,
					1.0F, 1.0F, 1.0F, 1.0F);
			if (flag) {
				List<Pair<Holder<BannerPattern>, DyeColor>> list = BannerBlockEntity
						.createPatterns(ShieldItem.getColor(stack), BannerBlockEntity.getItemPatterns(stack));
				BannerRenderer.renderPatterns(matrixStack, buffer, combinedLight, combinedOverlay,
						this.royalGuardShieldModel.plate(), rendermaterial, false, list, stack.hasFoil());
			} else {
				this.royalGuardShieldModel.plate().render(matrixStack, ivertexbuilder, combinedLight, combinedOverlay,
						1.0F, 1.0F, 1.0F, 1.0F);
			}
			...
			this.modelVanguardShield.getRoot().render(matrixStack, ivertexbuilder, combinedLight, combinedOverlay, 1.0F,
					1.0F, 1.0F, 1.0F);

			matrixStack.popPose();
		} else if (item instanceof ColoredTridentItem) {
			renderTridentEntityModel(stack, matrixStack, buffer, transformType, combinedLight, combinedOverlay,
					(ColoredTridentItem) item);

		}
	}

	private void renderTridentSpriteModel(ItemStack stack, PoseStack matrixStack, MultiBufferSource buffer,
			ItemTransforms.TransformType transformType, int combinedLight, int combinedOverlay, ColoredTridentItem item,
			boolean inHand) {
		matrixStack.pushPose();
		// matrixStack.translate(0.5, 0.5, 0.5);
		BakedModel model = Minecraft.getInstance().getModelManager()
				.getModel(getTridentMRL(item.getTridentColor(), inHand));
		Minecraft.getInstance().getItemRenderer().render(stack, transformType, false, matrixStack, buffer,
				combinedLight, combinedOverlay, model);
		matrixStack.popPose();
	}

	private void renderTridentEntityModel(ItemStack stack, PoseStack matrixStack, MultiBufferSource buffer,
			ItemTransforms.TransformType transformType, int combinedLight, int combinedOverlay,
			ColoredTridentItem item) {
		matrixStack.pushPose();
		matrixStack.scale(1.0F, -1.0F, -1.0F);
		ResourceLocation texture = getTridentTexture(item.getTridentColor());
		VertexConsumer foilBufferDirect = ItemRenderer.getFoilBufferDirect(buffer,
				this.tridentModel.renderType(texture), false, stack.hasFoil());
		this.tridentModel.renderToBuffer(matrixStack, foilBufferDirect, combinedLight, combinedOverlay, 1.0F, 1.0F,
				1.0F, 1.0F);
		matrixStack.popPose();
	}
