...
public abstract class AbstractVineEntity extends PathfinderMob implements Enemy, IAnimatable {
	...
	public static final EntityDataAccessor<Boolean> ALWAYS_OUT = SynchedEntityData.defineId(AbstractVineEntity.class,
			EntityDataSerializers.BOOLEAN);
	...
	public boolean shouldBurstFor(Entity entity) {
		return entity instanceof LivingEntity && !entity.getType().is(EntityTags.PLANT_MOBS) && entity.isAlive()
				&& !entity.isSpectator() && !(entity instanceof Player && ((Player) entity).isCreative());
	}
	...
	public EntityDimensions getDimensions(Pose p_213305_1_) {
		return !this.isOut() ? EntityDimensions.scalable(1.0F, 0.1F)
				: EntityDimensions.scalable(1.5F, ((float) this.getLengthInBlocks()) + this.getExtraHitboxY());
	}
	...
	public void baseTick() {
		...
		int nearbyEntities = this.level
				.getEntities(this, this.getBoundingBox().inflate(this.getDetectionDistance()), SHOULD_BURST_FOR).size();

		if (!this.level.isClientSide) {

			if (this.isInWrongHabitat() && this.random.nextInt(this.wrongHabitatDieChance()) == 0
					&& this.shouldDieInWrongHabitat() && this.isOut()) {
				this.kill();
			}
