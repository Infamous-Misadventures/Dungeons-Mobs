...
public class WhispererEntity extends Monster implements IAnimatable, IAquaticMob {
	...
	public static AttributeSupplier.Builder setCustomAttributes() {
		return Monster.createMonsterAttributes().add(Attributes.MOVEMENT_SPEED, 0.3D)
				.add(Attributes.ATTACK_DAMAGE, 6.0D).add(Attributes.FOLLOW_RANGE, 17.5D)
				.add(Attributes.MAX_HEALTH, 30.0D).add(Attributes.ARMOR, 5.0D)
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (this.attackAnimationTick > 0) {
			...
			if (this.isEyeInFluid(FluidTags.WATER) && this.isWavewhisperer()) {
				event.getController()
						.setAnimation(new AnimationBuilder().addAnimation("wavewhisperer_summon_pa", LOOP));
			} else {
			...
			if (this.isEyeInFluid(FluidTags.WATER) && this.isWavewhisperer()) {
				event.getController()
						.setAnimation(new AnimationBuilder().addAnimation("wavewhisperer_summon_qgk", LOOP));
			} else {
	...
	class BasicAttackGoal extends Goal {
		...
		public boolean canUse() {
			...
			return target != null && mob.distanceTo(target) <= 3 && animationsUseable() && mob.hasLineOfSight(target)
					&& mob.canUseGoals() && mob.random.nextInt(20) == 0;
		}
		...
		public void tick() {
			...
			if (target != null && mob.distanceTo(target) < 3.5 && mob.attackAnimationTick == actionPoint) {
	...
	class SummonQGVGoal extends Goal {
		...
		public boolean canUse() {
			target = mob.getTarget();
			return target != null && mob.distanceTo(target) <= 15 && animationsUseable() && mob.hasLineOfSight(target)
					&& mob.canUseGoals() && mob.tickCount >= this.nextUseTime;
		}
		...
		public void tick() {
			...
			if (target != null && mob.summonQGVAnimationTick == mob.summonQGVAnimationActionPoint) {
				...
				GeomancyHelper.summonAreaDenialVineTrap(mob, target,
						mob.isWavewhisperer() ? ModEntityTypes.QUICK_GROWING_KELP.get()
								: ModEntityTypes.QUICK_GROWING_VINE.get(),
						rowToRemove);
			}
		}
	...
	class SummonPQVAttackGoal extends Goal {
		...
		public boolean canUse() {
			target = mob.getTarget();
			return target != null && mob.distanceTo(target) <= 12.5 && animationsUseable() && mob.hasLineOfSight(target)
					&& mob.canUseGoals() && mob.tickCount >= this.nextUseTime;
		}
		...
		public void tick() {
			...
			if (target != null && (mob.summonPQVAnimationTick == mob.summonPQVAnimationActionPoint1
				...
				GeomancyHelper.summonOffensiveVine(mob, mob,
						mob.isWavewhisperer() ? ModEntityTypes.POISON_ANEMONE.get()
								: ModEntityTypes.POISON_QUILL_VINE.get(),
