...
public class SquallGolemEntity extends AbstractRaiderEntity implements IAnimatable {
    ...
    public static AttributeModifierMap.MutableAttribute setCustomAttributes() {
        return MonsterEntity.createMonsterAttributes()
                .add(Attributes.MAX_HEALTH, 125.0D) // 1x Golem Health
                .add(Attributes.MOVEMENT_SPEED, 0.24D)
                .add(Attributes.KNOCKBACK_RESISTANCE, 1.0D)
                .add(Attributes.ATTACK_DAMAGE, 21.0D) // 1x Golem Attack
    ...
    public boolean hurt(DamageSource source, float amount) {
        if (!this.getActivate() && !source.isBypassInvul()) {
            ...
            boolean flag = super.hurt(source,0);
        }
    ...
    class MeleeGoal extends Goal {
        ...
        @Override
        public void tick() {
            SquallGolemEntity.this.setDeltaMovement(Vector3d.ZERO);
            ...
            for (LivingEntity entityHit : level.getEntitiesOfClass(LivingEntity.class, getBoundingBox().inflate(X, Y, Z))) {
                ...
                if (entityHitDistance <= range && (entityRelativeAngle <= arc / 2 && entityRelativeAngle >= -arc / 2) || (entityRelativeAngle >= 360 - arc / 2 || entityRelativeAngle <= -360 + arc / 2)) {
                    if (!isAlliedTo(entityHit) &&!(entityHit instanceof SquallGolemEntity)) {
                        ...
                        entityHit.hurt(DamageSource.mobAttack(v), (float) (v.getAttributeValue(Attributes.ATTACK_DAMAGE) * 1.15F));
