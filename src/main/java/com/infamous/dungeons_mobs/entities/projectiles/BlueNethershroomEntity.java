	...
	protected void onHit(HitResult rtr) {
		...
		if (!list.isEmpty()) {
			if (!this.level.isClientSide) {
				...
				com.infamous.dungeons_mobs.client.util.ParticleGenerator.generatePotionImpact(this.level, potion,
						this.getItem(), blockPos, color, ModSoundEvents.FUNGUS_THROWER_FUNGUS_LAND.get());
			}
		}
	}

	private void makeAreaOfEffectCloud(@Nullable Entity target, ItemStack itemStack, Potion potion) {
		AreaEffectCloud aoeCloud = new AreaEffectCloud(this.level, target != null ? target.getX() : this.getX(),
				target != null ? target.getY() : this.getY(), target != null ? target.getZ() : this.getZ());
