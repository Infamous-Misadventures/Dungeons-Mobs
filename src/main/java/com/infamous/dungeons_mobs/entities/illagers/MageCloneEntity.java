...
public class MageCloneEntity extends AbstractIllager implements IAnimatable, SpawnArmoredMob {
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (this.appearAnimationTick > 0) {
			...
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("mage_shoot", EDefaultLoopTypes.LOOP));
	...
	class ShootAttackGoal extends Goal {
		...
		public boolean canUse() {
			...
			return target != null && !mob.shouldBeStationary() && mob.distanceTo(target) <= 16
					&& mob.distanceTo(target) > 5 && mob.hasLineOfSight(target) && animationsUseable();
		}
	...
	class CopyOwnerTargetGoal extends TargetGoal {
		...
		public boolean canUse() {
			return MageCloneEntity.this.owner != null && MageCloneEntity.this.owner.getTarget() != null
					&& this.canAttack(MageCloneEntity.this.owner.getTarget(), this.copyOwnerTargeting);
		}
