...
public class IceologerEntity extends AbstractIllager implements IAnimatable, SpawnArmoredMob {
	...
	public static AttributeSupplier.Builder setCustomAttributes() {
		return Monster.createMonsterAttributes().add(Attributes.MOVEMENT_SPEED, 0.25D).add(Attributes.FOLLOW_RANGE, 18D)
	...
	class SummonIceChunkGoal extends Goal {
		...
		public boolean canUse() {
			...
			int nearbyChunks = mob.level.getEntities(mob, mob.getBoundingBox().inflate(20.0D), ICE_CHUNK).size();

			return target != null && mob.random.nextInt(20) == 0 && mob.distanceTo(target) <= 12 && nearbyChunks <= 0
