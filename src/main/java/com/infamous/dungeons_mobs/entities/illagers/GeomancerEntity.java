...
public class GeomancerEntity extends SpellcasterIllager implements IAnimatable, SpawnArmoredMob {
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (this.summonBombsAttackAnimationTick > 0) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("geomancer_attack", EDefaultLoopTypes.LOOP));
		} else if (this.summonWallsAnimationTick > 0) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("geomancer_summon", EDefaultLoopTypes.LOOP));
		} else if (!(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F)) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("geomancer_walk", EDefaultLoopTypes.LOOP));
		} else {
			if (this.isCelebrating()) {
				event.getController().setAnimation(
						new AnimationBuilder().addAnimation("geomancer_celebrate", EDefaultLoopTypes.LOOP));
			} else {
				event.getController()
						.setAnimation(new AnimationBuilder().addAnimation("geomancer_idle", EDefaultLoopTypes.LOOP));
			}
		}
		...
		public void tick() {
			...
			if (target != null && mob.summonBombsAttackAnimationTick == mob.summonBombsAttackAnimationActionPoint) {
				...
				if (mob.getRandom().nextBoolean()) {
					...
					GeomancyHelper.summonOffensiveConstruct(mob, target, ModEntityTypes.GEOMANCER_BOMB.get(),
							movingOnX ? (mob.random.nextBoolean() ? 2 : -2) : 0,
							!movingOnX ? (mob.random.nextBoolean() ? 2 : -2) : 0, Direction.NORTH);
				}
			}
		}
