...
public class DungeonsEvokerEntity extends SpellcastingIllagerEntity implements IAnimatable {
    ...
    protected void registerGoals() {
        super.registerGoals();
		this.goalSelector.addGoal(5, new AvoidEntityGoal<>(this, VexEntity.class, 1.0F, 0.7D, 1.05D));
    ...
    public void baseTick() {
		...
		for(Entity entity : list) {
			if(entity instanceof VexEntity){
				...
				if (livingEntity.getOwner() == this) {
					this.liftInterval =60+this.getRandom().nextInt(2);
					this.duplicateInterval =60+this.getRandom().nextInt(2);
				}
			}
		}
			...
			if (mob.isAngry() == 17) {
				...
				if (mob.getTarget() != null) {
					if (DungeonsEvokerEntity.this.distanceToSqr(livingentity) < 60.0D && mob.distanceToSqr(livingentity) > 30) {
						...
						for (k = 0; k < 24; ++k) {
							f2 = f + (float)k * (float)Math.PI * 5.0F / 24.0F + 6.2566371F;
							this.createSpellEntity(DungeonsEvokerEntity.this.getX() + (double)MathHelper.cos(f2) * 5.5D, DungeonsEvokerEntity.this.getZ() + (double)MathHelper.sin(f2) * 5.5D, d0, d1, f2, k / 36);
						}
