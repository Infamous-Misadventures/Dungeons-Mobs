...
public class IllagerArmsUtil {
	public static boolean armorHasCrossedArms(AbstractIllager p_241739_3_, ItemStack itemstack) {
		return !(itemstack.getItem() instanceof ArmorItem)
				|| resourceExists(getArmorResourceStatic(p_241739_3_, itemstack, EquipmentSlot.CHEST));
	}
	...
	private static ResourceLocation getArmorResourceStatic(Entity entity, ItemStack stack, EquipmentSlot slot) {
		...
		String defaultString = String.format("%s:textures/models/armor/%s_layer_%d%s.png", domain, texture, 1,
				"crossed" == null ? "" : String.format("_%s", "crossed"));

		String s1 = net.minecraftforge.client.ForgeHooksClient.getArmorTexture(entity, stack, defaultString, slot,
				"crossed");
