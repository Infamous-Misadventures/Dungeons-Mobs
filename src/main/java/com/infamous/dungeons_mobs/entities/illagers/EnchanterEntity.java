...
public class EnchanterEntity extends SpellcasterIllager implements IAnimatable {
	...
	public static final EntityDataAccessor<Integer> ATTACK_TICKS = SynchedEntityData.defineId(EnchanterEntity.class,
			EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> ENCHANT_TICKS = SynchedEntityData.defineId(EnchanterEntity.class,
			EntityDataSerializers.INT);
	public static final EntityDataAccessor<List<UUID>> ENCHANTMENT_TARGETS = SynchedEntityData
			.defineId(EnchanterEntity.class, UUID_LIST.get());
	...
	public static AttributeSupplier.Builder setCustomAttributes() {
		return Monster.createMonsterAttributes().add(Attributes.ATTACK_DAMAGE, 8.0D)
				.add(Attributes.MOVEMENT_SPEED, 0.2D).add(Attributes.FOLLOW_RANGE, 20.0D)
	...
	public void addEnchantmentTarget(Monster monsterEntity) {
		...
		this.entityData.set(ENCHANTMENT_TARGETS,
				enchantmentTargets.stream().map(Entity::getUUID).collect(Collectors.toList()));
	}
	...
	public void baseTick() {
		super.baseTick();
		if (this.level.getNearestPlayer(this, 2) != null && this.getTarget() != null
				&& this.level.getNearestPlayer(this, 2) == this.getTarget()) {
		...
		List<Monster> validEnchantmentTargets = this.enchantmentTargets.stream().filter(this::isValidEnchantmentTarget)
				.collect(Collectors.toList());
		this.enchantmentTargets.stream().filter(monsterEntity -> !validEnchantmentTargets.contains(monsterEntity))
				.filter(LivingEntity::isAlive).forEach(this::clearEntityMobEnchantments);
	...
	class CastingSpellGoal extends SpellcasterIllager.SpellcasterCastingSpellGoal {
		...
		public void tick() {
			if (EnchanterEntity.this.getEnchantmentTarget() != null) {
				EnchanterEntity.this.getLookControl().setLookAt(EnchanterEntity.this.getEnchantmentTarget(),
						(float) EnchanterEntity.this.getMaxHeadYRot(), (float) EnchanterEntity.this.getMaxHeadXRot());
			}
...
//        private final EntityPredicate wololoTargeting = (new EntityPredicate()).range(16.0D).selector();
		...
		public boolean canUse() {
			if (EnchanterEntity.this.getTarget() == null) {
				...
				List<LivingEntity> list = AreaOfEffectHelper.getNearbyEnemies(EnchanterEntity.this, 16,
						EnchanterEntity.this.level, livingEntity -> {
							if (livingEntity.getCapability(EnchantWithMob.MOB_ENCHANT_CAP).isPresent()) {
								MobEnchantCapability mobEnchantCapability = livingEntity
										.getCapability(EnchantWithMob.MOB_ENCHANT_CAP).resolve().get();
				...
				if (list.isEmpty()) {
					...
					EnchanterEntity.this
							.setEnchantmentTarget((Monster) list.get(EnchanterEntity.this.random.nextInt(list.size())));
	...
	class AttackingGoal extends Goal {
		...
		public void tick() {
			...
			EnchanterEntity.this.getLookControl().setLookAt(EnchanterEntity.this.getTarget().getX(),
					EnchanterEntity.this.getTarget().getEyeY(), EnchanterEntity.this.getTarget().getZ());
		}
	}
