...
public class WindcallerEntity extends AbstractIllager implements IAnimatable, SpawnArmoredMob {
	...
	public void baseTick() {
		...
		if (this.getTarget() != null && this.distanceTo(this.getTarget()) > 4
				&& ((!this.isOnGround() && steepDropBelow())
						|| ((this.getTarget().getY() > this.getY() + 4 || this.getY() < this.getTarget().getY() - 2)
	...
	public void aiStep() {
		...
		if (this.level.isClientSide) {
			this.level.addParticle(ModParticleTypes.WIND.get(), this.getRandomX(0.1D), this.getY() + 0.05D,
					this.getRandomZ(0.1D), (this.random.nextDouble() - 0.5D) * 1.0D, 0.0,
	...
	public static AttributeSupplier.Builder setCustomAttributes() {
		return Monster.createMonsterAttributes().add(Attributes.MOVEMENT_SPEED, 0.35D).add(Attributes.FOLLOW_RANGE, 20D)
	...
	class LiftAttackGoal extends Goal {
		...
		public boolean canUse() {
			...
			if (target != null) {
				nearbyTornadoes = mob.level.getEntities(mob, target.getBoundingBox().inflate(5.0D), TORNADO).size();
			}

			return target != null && target.isOnGround() && mob.random.nextInt(30) == 0 && mob.distanceTo(target) <= 16
					&& mob.distanceTo(target) > 3 && nearbyTornadoes <= 0 && mob.hasLineOfSight(target)
					&& animationsUseable();
		}
	...
	class BlastAttackGoal extends Goal {
		...
		public boolean canUse() {
			...
			return target != null && mob.random.nextInt(5) == 0
					&& (mob.distanceTo(target) <= 4
							|| (!target.isOnGround() && mob.random.nextInt(10) == 0 && mob.distanceTo(target) < 7.5))
