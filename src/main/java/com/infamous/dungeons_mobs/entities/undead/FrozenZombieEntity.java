...
public class FrozenZombieEntity extends Zombie implements RangedAttackMob {
	...
	public static boolean canFrozenZombieSpawn(EntityType<FrozenZombieEntity> entityType, ServerLevelAccessor iWorld,
	...
	public boolean isAlliedTo(Entity entityIn) {
		if (super.isAlliedTo(entityIn)) {
			return true;
		} else if (entityIn instanceof LivingEntity && ((LivingEntity) entityIn) instanceof Zombie) {
	...
	public void aiStep() {
		if (this.level.isClientSide) {
			this.level.addParticle(ModParticleTypes.SNOWFLAKE.get(), this.getRandomX(0.5D), this.getRandomY() - 0.25D,
			...
			if (targetEntity instanceof LivingEntity) {
				...
				if (i > 0) {
					((LivingEntity) targetEntity)
							.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, i * 20, 0));
				}
			}
	...
	public void performRangedAttack(LivingEntity livingEntity, float v) {
		...
		float adjustedHorizontalDistance = Mth.sqrt((float) (xDifference * xDifference + zDifference * zDifference))
				* 0.2F;
	...
	static class FrozenZombieAttackGoal extends ThrowAndMeleeAttackGoal {
		...
		public boolean hasThrowableItemInMainhand() {
			return (this.zombie.getMainHandItem().getItem() instanceof SnowballItem
					| this.zombie.getMainHandItem().getItem() instanceof EggItem) && zombie.getTarget() != null
