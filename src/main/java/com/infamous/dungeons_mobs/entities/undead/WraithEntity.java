...
public class WraithEntity extends Monster implements IAnimatable {
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(3,
				new NearestAttackableTargetGoal<>(this, IronGolem.class, true).setUnseenMemoryTicks(300));
		this.targetSelector.addGoal(3,
	...
	public void baseTick() {
		...
		if (this.teleportAnimationTick > 0) {
			this.level.addParticle(ParticleTypes.SOUL_FIRE_FLAME, this.getRandomX(1), this.getY(), this.getRandomZ(1),
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (this.summonFireAttackAnimationTick > 0) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("wraith_attack", EDefaultLoopTypes.LOOP));
		} else if (this.teleportAnimationTick > 10) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("wraith_teleport", EDefaultLoopTypes.LOOP));
		} else if (!(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F)) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("wraith_fly", EDefaultLoopTypes.LOOP));
		} else {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("wraith_idle", EDefaultLoopTypes.LOOP));
		}
	...
	private boolean teleport(double p_70825_1_, double p_70825_3_, double p_70825_5_) {
		...
		if (flag && !flag1) {
			...
			if (flag2 && !this.isSilent()) {
				this.level.playSound((Player) null, this.xo, this.yo, this.zo, ModSoundEvents.WRAITH_TELEPORT.get(),
	...
	class TeleportGoal extends Goal {
		...
		public boolean canUse() {
			...
			return target != null && (mob.distanceTo(target) <= 4 || mob.distanceTo(target) >= 16)
		...
		public void tick() {
			...
			if (target != null && mob.teleportAnimationTick == mob.teleportAnimationActionPoint) {
				if (mob.distanceTo(target) >= 16) {
					for (int i = 0; i < 10; i++) {
						mob.teleport(target.getX() - teleportToRange + mob.random.nextInt(teleportToRange * 2),
								target.getY(),
								target.getZ() - teleportToRange + mob.random.nextInt(teleportToRange * 2));
					}
				} else {
					for (int i = 0; i < 10; i++) {
						mob.teleport(target.getX() - teleportAwayRange + mob.random.nextInt(teleportAwayRange * 2),
								target.getY(),
								target.getZ() - teleportAwayRange + mob.random.nextInt(teleportAwayRange * 2));
					}
				}
