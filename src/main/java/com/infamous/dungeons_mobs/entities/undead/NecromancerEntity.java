...
public class NecromancerEntity extends Skeleton implements IAnimatable, SpawnArmoredMob {
	...
	public static AttributeSupplier.Builder setCustomAttributes() {
		return Skeleton.createAttributes().add(Attributes.MOVEMENT_SPEED, 0.2D).add(Attributes.FOLLOW_RANGE, 20.0D)
				.add(Attributes.MAX_HEALTH, 40.0D).add(Attributes.ARMOR, 5.0D)
				.add(Attributes.KNOCKBACK_RESISTANCE, 0.4D).add(AttributeRegistry.SUMMON_CAP.get(), 4);
	}
	...
	class SummonGoal extends Goal {
		...
		public void tick() {
			...
			if (target != null && (mob.summonAnimationTick == mob.summonAnimationActionPoint1
