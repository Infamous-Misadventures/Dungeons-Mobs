...
public class FungusThrowerAi {
	...
	public static void performFungusThrow(LivingEntity fungusThrower, LivingEntity attackTarget) {
		...
		if (!fungusThrower.isSilent()) {
			fungusThrower.level.playSound((Player) null, fungusThrower.getX(), fungusThrower.getY(),
					fungusThrower.getZ(), ModSoundEvents.FUNGUS_THROWER_THROW.get(), fungusThrower.getSoundSource(),
					1.0F,
					(fungusThrower.getRandom().nextFloat() - fungusThrower.getRandom().nextFloat()) * 0.2F + 1.0F);
		}
