...
public class IllagerWarriorEntity extends SpellcastingIllagerEntity implements IAnimatable {
	...
	public ILivingEntityData finalizeSpawn(IServerWorld p_213386_1_, DifficultyInstance p_213386_2_, SpawnReason reason, @Nullable ILivingEntityData p_213386_4_, @Nullable CompoundNBT p_213386_5_) {
		this.remove();
		return super.finalizeSpawn(p_213386_1_, p_213386_2_, reason, p_213386_4_, p_213386_5_);
	}
	    ...
	    public void tick() {
			...
			if (mob.getLiftTicks() >= 40) {
				if (mob.timer == 0) {
					...
					for (k = 0; k < o; ++k) {
						f2 = f + (float) k * 3.1415927F * 2.0F / o + (1.2566371F + (o - 5) / 3F);
						this.createSpellEntity(IllagerWarriorEntity.this.getX() + (double) MathHelper.cos(f2) * (1.5D + (o - 5) / 3D), IllagerWarriorEntity.this.getZ() + (double) MathHelper.sin(f2) * (1.5D + (o - 5) / 3D), d0, d1, f2, (int) (o - 3));
					}
			...
			if (mob.getLiftTicks() == 32) {
				...
				int mobsToSummon = difficultyAsInt + 2; // 3 on easy, 4 on normal, 5 on hard
	    ...
	    public void tick() {
          ...
          if (mob.getDuplicateTicks() == 18) {
           ...
           private void summonIllusionerClones(){
			   ServerWorld serverworld = (ServerWorld) IllagerWarriorEntity.this.level;
              int difficultyAsInt = IllagerWarriorEntity.this.level.getDifficulty().getId();
              int mobsToSummon = difficultyAsInt * 2 + 1; // 6 on easy, 9 on normal, 12 on hard
              for(int i = 0; i < mobsToSummon; ++i) {
				  BlockPos blockpos = IllagerWarriorEntity.this.blockPosition().offset(-2 + IllagerWarriorEntity.this.random.nextInt(5), 1, -2 + IllagerWarriorEntity.this.random.nextInt(5));
				  VexEntity vexentity = (VexEntity)EntityType.VEX.create(IllagerWarriorEntity.this.level);
				  vexentity.moveTo(blockpos, 0.0F, 0.0F);
	...
	class CloneGoal extends Goal {
		...
		private void summonIllusionerClones(){
			...
			for(int i = 0; i < mobsToSummon; ++i) {
				...
				IllagerWarriorCloneEntity illusionerCloneEntity = new IllagerWarriorCloneEntity(IllagerWarriorEntity.this.level, 50, true,IllagerWarriorEntity.this.getTarget());
				...
				illusionerCloneEntity.spellInterval = i * 5;
	...
	class SCloneGoal extends Goal {
		...
		private void summonIllusionerClones(){
			int difficultyAsInt = IllagerWarriorEntity.this.level.getDifficulty().getId();
			int mobsToSummon = difficultyAsInt * 3 + 4; // 7 on easy, 10 on normal, 13 on hard
			for(int i = 0; i < mobsToSummon; ++i) {
