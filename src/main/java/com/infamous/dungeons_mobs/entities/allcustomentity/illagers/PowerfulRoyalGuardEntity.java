...
public class PowerfulRoyalGuardEntity extends AbstractIllagerEntity implements IAnimatable {
    ...
    private void applyDiamondArmorBoosts() {
        ...
        this.getAttribute(Attributes.ARMOR).addPermanentModifier(new AttributeModifier("Diamond armor boost", 5.0D, AttributeModifier.Operation.ADDITION));
        ...
        this.getAttribute(Attributes.ATTACK_DAMAGE).addPermanentModifier(new AttributeModifier("Diamond attack boost", 5.0D, AttributeModifier.Operation.ADDITION));
    }
    ...
    class MeleeGoal extends Goal {
        ...
        public void tick() {
            if (PowerfulRoyalGuardEntity.this.getTarget() != null && PowerfulRoyalGuardEntity.this.getTarget().isAlive()) {
                ...
                if (attackTimer == 15 && PowerfulRoyalGuardEntity.this.distanceToSqr(PowerfulRoyalGuardEntity.this.getTarget()) <= 7.5) {
    ...
    protected void hurtCurrentlyUsedShield(float amount) {
        if (this.useItem.isShield(this)) {
            if (amount >= 3.0F) {
                ...
                if (this.isDiamond())
                    i = (int) (1 + MathHelper.floor(amount) * 2);

                if (this.isGold())
                    i = (int) (1 + MathHelper.floor(amount) * 5);
