...
public class AreaDamageEntity extends Entity {
	...
	public static AreaDamageEntity spawnAreaDamage(Level level, Vec3 pos, LivingEntity owner, float damage,
			DamageSource damageSource, float size, float sizeToReach, float growSpeed, float ySize, int extraTime,
			boolean constantDamage, boolean friendlyFire, double knockbackAmount, double knockbackAmountY,
			boolean disableShields, int disableShieldTime, int particleVariant) {
	...
	public void baseTick() {
		...
		if (!this.level.isClientSide && ((this.getExtraTime() > 0 && this.extraTimeTick >= this.getExtraTime())
