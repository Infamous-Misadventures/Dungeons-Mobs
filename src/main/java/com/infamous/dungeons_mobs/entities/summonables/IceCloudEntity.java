    ...
    private void tryToFloatAboveTarget(LivingEntity targetIn) {
        ...
        if(nearbyIceClouds.isEmpty()) {
            ...
            this.setDeltaMovement(getDeltaMovement().add(
                    1 * Math.max(Math.min((targetIn.getX() - this.getX()) / 40, 0.15), -0.15) / 6,
                    0,
                    1 * Math.max(Math.min((targetIn.getZ() - this.getZ()) / 40, 0.15), -0.15) / 6
            ));
            this.setDeltaMovement(getDeltaMovement().add(
                    0,
                    1 * Math.max(Math.min((targetIn.getY() + targetIn.getBbHeight() + 0.5 - this.getY() ) / 40, 0.05), 0),
                    0
            ));
        }
    }
