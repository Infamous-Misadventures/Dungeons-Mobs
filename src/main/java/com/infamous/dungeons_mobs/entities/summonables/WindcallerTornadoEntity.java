		...
		if (!this.isBlast()) {
			...
			if (this.lifeTime >= 15 && this.lifeTime < 30) {
				if (this.level.isClientSide) {
					for (int i = 0; i < 3; i++) {
						this.level.addParticle(ModParticleTypes.WIND.get(), this.getRandomX(0.5D),
								this.getRandomY() - 2, this.getRandomZ(0.5D), (this.random.nextDouble() - 0.5D) * 1.0D,
								5, (this.random.nextDouble() - 0.5D) * 1.0D);
					}
				}
			}
		}
