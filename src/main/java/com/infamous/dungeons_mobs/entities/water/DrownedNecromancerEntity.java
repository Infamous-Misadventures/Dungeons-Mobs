...
public class DrownedNecromancerEntity extends DrownedEntity implements IAnimatable {
	...
	public static AttributeModifierMap.MutableAttribute setCustomAttributes() {
		return DrownedEntity.createAttributes().add(Attributes.MOVEMENT_SPEED, 0.2D).add(ForgeMod.SWIM_SPEED.get(), 2.5D).add(Attributes.FOLLOW_RANGE, 30.0D).add(Attributes.MAX_HEALTH, 75.0D).add(Attributes.ARMOR, 12.5D).add(Attributes.KNOCKBACK_RESISTANCE, 0.6D);
	}
	...
	class LandSummonGoal extends Goal {
		...
		public void tick() {
			...
			if (target != null && 
		            ...
		            if (summonedMob == null) {
		                summonedMob = EntityType.ZOMBIE.create(mob.level);
		            }
		...
		public void tick() {
			...
			if (target != null && 
				...
				for (int i = 0; i < 6; i++) {
	            	...
	            	summonedMob.setTarget(target);
	            	summonedMob.finalizeSpawn(((ServerWorld)mob.level), mob.level.getCurrentDifficultyAt(summonPos), SpawnReason.MOB_SUMMONED, (ILivingEntityData)null, (CompoundNBT)null);
	            	mobSummonSpot.playSound(ModSoundEvents.DROWNED_NECROMANCER_SUMMON.get(), 1.75F, 1.0F);
					if (mob.getTeam() != null) {
						Scoreboard scoreboard = mob.level.getScoreboard();
						scoreboard.addPlayerToTeam(summonedMob.getScoreboardName(), scoreboard.getPlayerTeam(mob.getTeam().getName()));
					}
