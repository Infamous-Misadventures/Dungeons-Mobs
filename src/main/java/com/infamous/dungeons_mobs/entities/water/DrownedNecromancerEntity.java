...
public class DrownedNecromancerEntity extends Drowned implements IAnimatable, SpawnArmoredMob {
	...
	public static AttributeSupplier.Builder setCustomAttributes() {
		return Drowned.createAttributes().add(Attributes.MOVEMENT_SPEED, 0.2D).add(ForgeMod.SWIM_SPEED.get(), 2.5D)
				.add(Attributes.FOLLOW_RANGE, 30.0D).add(Attributes.MAX_HEALTH, 75.0D).add(Attributes.ARMOR, 12.5D)
	...
	public boolean isAlliedTo(Entity entityIn) {
		if (super.isAlliedTo(entityIn)) {
			return true;
		} else if (entityIn instanceof LivingEntity && ((LivingEntity) entityIn).getMobType() == MobType.UNDEAD) {
	...
	private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
		if (this.tridentStormAnimationTick > 0) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("drowned_necromancer_trident_storm", LOOP));
		} else if (this.rainTridentStormAnimationTick > 0) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("drowned_necromancer_trident_storm_land", LOOP));
			...
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("drowned_necromancer_shoot_land", LOOP));
			...
			if (this.isInWater()) {
				event.getController()
						.setAnimation(new AnimationBuilder().addAnimation("drowned_necromancer_swim", LOOP));
			} else {
			...
			if (this.isInWater()) {
				event.getController()
						.setAnimation(new AnimationBuilder().addAnimation("drowned_necromancer_idle", LOOP));
			} else {
	...
	private boolean isInRain() {
		...
		return this.level.isRainingAt(blockpos) || this.level.isRainingAt(
	...
	static class GoToBeachGoal extends MoveToBlockGoal {
		...
		protected boolean isValidTarget(LevelReader p_179488_1_, BlockPos p_179488_2_) {
			...
			return p_179488_1_.isEmptyBlock(blockpos) && p_179488_1_.isEmptyBlock(blockpos.above())
					? p_179488_1_.getBlockState(p_179488_2_).entityCanStandOn(p_179488_1_, p_179488_2_, this.drowned)
					: false;
		}
	...
	class LandSummonGoal extends Goal {
		...
		public void tick() {
			...
			if (target != null && (mob.landSummonAnimationTick == mob.landSummonAnimationActionPoint1
				...
				if (mobSummonSpot.isInWall() || !canSee(mobSummonSpot, target)) {
					summonPos = mob.blockPosition().offset(
							-closeMobSummonRange + mob.random.nextInt((closeMobSummonRange * 2) + 1), 0,
							-closeMobSummonRange + mob.random.nextInt((closeMobSummonRange * 2) + 1));
				}
				...
				summonedMob.finalizeSpawn(((ServerLevel) mob.level), mob.level.getCurrentDifficultyAt(summonPos),
		...
		public boolean canSee(Entity entitySeeing, Entity p_70685_1_) {
			...
			return entitySeeing.level.clip(new ClipContext(vector3d, vector3d1, ClipContext.Block.COLLIDER,
					ClipContext.Fluid.NONE, entitySeeing)).getType() == HitResult.Type.MISS;
		}
	...
	class LandShootAttackGoal extends Goal {
		...
		public boolean canUse() {
			target = mob.getTarget();
			return target != null && mob.distanceTo(target) <= 12.5 && mob.hasLineOfSight(target) && animationsUseable()
					&& !mob.isInWaterRainOrBubble();
		}
	...
	class RainTridentStormGoal extends Goal {
		...
		public boolean canUse() {
			...
			return target != null && mob.tickCount >= this.nextUseTime && mob.distanceTo(target) <= 13
					&& animationsUseable() && mob.hasLineOfSight(target) && !mob.isInWaterOrBubble() && mob.isInRain();
		}
		...
		public void tick() {
			...
			if (target != null && mob.rainTridentStormAnimationTick <= 30 && mob.rainTridentStormAnimationTick >= 10
					&& mob.random.nextBoolean()) {
				...
				tridentStorm.moveTo(new BlockPos(
						mob.getX() - tridentSummonRange + mob.random.nextInt(tridentSummonRange * 2), mob.getY(),
						mob.getZ() - tridentSummonRange + mob.random.nextInt(tridentSummonRange * 2)), 0, 0);
	...
	class RainShootAttackGoal extends Goal {
		...
		public boolean canUse() {
			target = mob.getTarget();
			return target != null && mob.distanceTo(target) <= 12.5 && mob.hasLineOfSight(target) && animationsUseable()
					&& !mob.isInWaterOrBubble() && mob.isInRain();
		}
		...
		public void tick() {
			...
			if (target != null && (mob.rainShootAnimationTick == mob.rainShootAnimationActionPoint
				...
				double d2 = target.getY(0.6D) - pos.y;
				double d3 = target.getZ() - pos.z;
				DrownedNecromancerOrbEntity necromancerOrb = new DrownedNecromancerOrbEntity(mob.level, mob,
						d1 + (mob.random.nextGaussian() * 1.0), d2 + (mob.random.nextGaussian() * 0.25),
	...
	class ShootAttackGoal extends Goal {
		...
		public void tick() {
			...
			if (target != null && (mob.shootAnimationTick == mob.shootAnimationActionPoint
				...
				double d3 = target.getZ() - pos.z;
				DrownedNecromancerOrbEntity necromancerOrb = new DrownedNecromancerOrbEntity(mob.level, mob,
						d1 + (mob.random.nextGaussian() * 1.25), d2 + (mob.random.nextGaussian() * 0.5),
	...
	class TridentStormGoal extends Goal {
		...
		public boolean canUse() {
			...
			return target != null && mob.tickCount >= this.nextUseTime && mob.distanceTo(target) <= 15
					&& animationsUseable() && mob.hasLineOfSight(target) && mob.isInWaterOrBubble();
		}
