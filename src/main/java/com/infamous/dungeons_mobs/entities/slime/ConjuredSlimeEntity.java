...
public class ConjuredSlimeEntity extends Slime implements RangedAttackMob {
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(1,
				new NearestAttackableTargetGoal<>(this, Player.class, 10, true, false, this::canSlimeReach));
	...
	public void performRangedAttack(LivingEntity target, float distanceFactor) {
		...
		float euclidDist = Mth
				.sqrt((float) (xDifference * xDifference + yDifference * yDifference + zDifference * zDifference));

		SlimeballEntity slimeballEntity = new SlimeballEntity(this.level, this, 0, 0, 0);
		slimeballEntity.setPos(slimeballEntity.getX(), this.getY(0.5D), slimeballEntity.getZ());
		...
		public void tick() {
			...
			if (attackTarget != null) {
				...
				if (--this.rangedAttackTime == 0) {
					...
					this.rangedAttackTime = Mth
							.floor(f * (float) (this.maxRangedAttackTime - this.minRangedAttackInterval)
									+ (float) this.minRangedAttackInterval);
				} else if (this.rangedAttackTime < 0) {
					float f2 = Mth.sqrt((float) distanceSqToTarget) / this.attackRadius;
					this.rangedAttackTime = Mth
							.floor(f2 * (float) (this.maxRangedAttackTime - this.minRangedAttackInterval)
									+ (float) this.minRangedAttackInterval);
				}
	...
	static class FaceRandomGoal extends Goal {
		...
		public void tick() {
			...
			((ConjuredSlimeEntity.MoveHelperController) this.slime.getMoveControl()).setDirection(this.chosenDegrees);
		}
	}
