...
public class CustomDrownedRenderer extends AbstractZombieRenderer<Drowned, DrownedModel<Drowned>> {
	...
	private static final ResourceLocation SEAWEED_ARMORED_DROWNED_LOCATION = new ResourceLocation(DungeonsMobs.MODID,
			"textures/entity/ocean/seaweed_armored_drowned.png");
	private static final ResourceLocation PALE_ARMORED_DROWNED_LOCATION = new ResourceLocation(DungeonsMobs.MODID,
			"textures/entity/ocean/pale_armored_drowned.png");
	private static final List<ResourceLocation> ARMORED_DROWNED_LOCATIONS = Arrays
			.asList(SEAWEED_ARMORED_DROWNED_LOCATION, PALE_ARMORED_DROWNED_LOCATION);

	public CustomDrownedRenderer(EntityRendererProvider.Context rendererContext) {
		super(rendererContext, new DrownedModel<>(rendererContext.bakeLayer(ModelLayers.DROWNED)),
				new DrownedModel<>(rendererContext.bakeLayer(ModelLayers.DROWNED_INNER_ARMOR)),
				new DrownedModel<>(rendererContext.bakeLayer(ModelLayers.DROWNED_OUTER_ARMOR)));
		this.addLayer(new CustomDrownedOuterLayer<>(this,
				new DrownedModel<>(rendererContext.bakeLayer(ModelLayers.DROWNED_OUTER_LAYER))));
	}
	...
	@Override
	protected void setupRotations(Drowned drowned, PoseStack matrixStack, float p_225621_3_, float p_225621_4_,
			float p_225621_5_) {
		...
		if (swimAmount > 0.0F) {
			matrixStack.mulPose(
					Vector3f.XP.rotationDegrees(Mth.lerp(swimAmount, drowned.getXRot(), -10.0F - drowned.getXRot())));
		}
	}
