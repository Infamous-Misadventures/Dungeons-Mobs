...
public class CustomPiglinRenderer extends PiglinRenderer {
	...
	public CustomPiglinRenderer(EntityRendererProvider.Context rendererContext, ModelLayerLocation p_174345_,
			ModelLayerLocation p_174346_, ModelLayerLocation p_174347_, boolean isZombified, boolean isFungusThrower) {
		super(rendererContext, p_174345_, p_174346_, p_174347_, isZombified);
		if (isFungusThrower) {
			this.addLayer(new FungusSackLayer<>(this,
					new FungusSackModel<Mob>(rendererContext.bakeLayer(ModModelLayers.FUNGUS_SACK))));
		}
	}
	...
	public ResourceLocation getTextureLocation(Mob mobEntity) {
		...
		return RESOURCE_LOCATION_MAP.computeIfAbsent(mobEntity.getType(), type -> new HashMap<>()).computeIfAbsent(
				skinVariantName, s -> new ResourceLocation(DungeonsMobs.MODID, this.getPath(skinVariantName)));
	}
