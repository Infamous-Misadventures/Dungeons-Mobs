...
public class SunkenSkeletonRenderer<T extends SunkenSkeletonEntity>
	...
	private static final ResourceLocation SUNKEN_SKELETON_LOCATION = new ResourceLocation(DungeonsMobs.MODID,
			"textures/entity/ocean/sunken_skeleton.png");
	private static final ResourceLocation RED_CORAL_ARMORED_SUNKEN_SKELETON_LOCATION = new ResourceLocation(
			DungeonsMobs.MODID, "textures/entity/ocean/red_coral_armored_sunken_skeleton.png");
	private static final ResourceLocation YELLOW_CORAL_ARMORED_SUNKEN_SKELETON_LOCATION = new ResourceLocation(
			DungeonsMobs.MODID, "textures/entity/ocean/yellow_coral_armored_sunken_skeleton.png");
	private static final List<ResourceLocation> ARMORED_SKELETON_LOCATIONS = Arrays
			.asList(RED_CORAL_ARMORED_SUNKEN_SKELETON_LOCATION, YELLOW_CORAL_ARMORED_SUNKEN_SKELETON_LOCATION);
	...
	public SunkenSkeletonRenderer(EntityRendererProvider.Context renderContext) {
		...
		this.addLayer(
				new HumanoidArmorLayer<>(this, new SunkenSkeletonModel<>(renderContext.bakeLayer(SKELETON_INNER_ARMOR)),
						new SunkenSkeletonModel<>(renderContext.bakeLayer(SKELETON_OUTER_ARMOR))));
	}
	...
	protected void setupRotations(T skeleton, PoseStack matrixStack, float p_225621_3_, float p_225621_4_,
		...
		if (swimAmount > 0.0F) {
			matrixStack.mulPose(
					Vector3f.XP.rotationDegrees(Mth.lerp(swimAmount, skeleton.getXRot(), -10.0F - skeleton.getXRot())));
		}
	}
