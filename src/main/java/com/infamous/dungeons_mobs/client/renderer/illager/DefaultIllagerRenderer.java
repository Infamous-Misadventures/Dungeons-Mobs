...
public class DefaultIllagerRenderer<T extends Mob & IAnimatable> extends ExtendedGeoEntityRenderer<T> {
	...
	protected void prepareArmorPositionAndScale(GeoBone bone, List<ModelPart.Cube> cubeList, ModelPart sourceLimb,
		...
		if (bone.getName().equals("armorBipedHead") && geoArmor
				&& ForgeRegistries.ITEMS.getKey(helmet.getItem()).getNamespace().equals(MODID)) {
			stack.translate(0, 0.125, 0); // 1y is 1 cube up, we want 2/16
		}
	}

	@Override
	protected void setLimbBoneVisible(GeoArmorRenderer<? extends GeoArmorItem> armorRenderer, ModelPart limb,
		...
		if (limb == armorModel.head || limb == armorModel.hat) {
			...
			armorRenderer.getGeoModelProvider()
					.getBone((slot == EquipmentSlot.FEET ? armorRenderer.leftBootBone : armorRenderer.leftLegBone))
					.setHidden(false);
			armorRenderer.getGeoModelProvider()
					.getBone((slot == EquipmentSlot.FEET ? armorRenderer.leftLegBone : armorRenderer.leftBootBone))
					.setHidden(true);
			...
			armorRenderer.getGeoModelProvider()
					.getBone((slot == EquipmentSlot.FEET ? armorRenderer.rightBootBone : armorRenderer.rightLegBone))
					.setHidden(false);
			armorRenderer.getGeoModelProvider()
					.getBone((slot == EquipmentSlot.FEET ? armorRenderer.rightLegBone : armorRenderer.rightBootBone))
					.setHidden(true);
