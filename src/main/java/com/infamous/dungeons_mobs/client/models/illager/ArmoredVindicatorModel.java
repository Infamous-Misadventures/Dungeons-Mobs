...
public class ArmoredVindicatorModel extends AnimatedGeoModel  {
		...
		public ResourceLocation getTextureLocation(Object entity) {
			return (entity instanceof ArmoredVindicatorEntity && ((ArmoredVindicatorEntity)entity).isDiamond()) ?
					new ResourceLocation(DungeonsMobs.MODID, "textures/geo_entity/illager/diamond_armored_vindicator.png") :
					(entity instanceof ArmoredVindicatorEntity && ((ArmoredVindicatorEntity)entity).isGold()) ?
							new ResourceLocation(DungeonsMobs.MODID, "textures/geo_entity/illager/gold_armored_vindicator.png"):
							new ResourceLocation(DungeonsMobs.MODID, "textures/geo_entity/illager/iron_armored_vindicator.png");
		}
	...
	public void setLivingAnimations(IAnimatable entity, Integer uniqueID, AnimationEvent customPredicate) {
		...
		IBone c = this.getAnimationProcessor().getBone("armored");
		IBone v = this.getAnimationProcessor().getBone("humanoid");

		EntityModelData extraData = (EntityModelData) customPredicate.getExtraDataOfType(EntityModelData.class).get(0);

		if (extraData.headPitch != 0 || extraData.netHeadYaw != 0) {
			((ArmoredVindicatorEntity)entityIn).setH(true);
			if (!((ArmoredVindicatorEntity)entityIn).getG()) {
			...
			((ArmoredVindicatorEntity)entityIn).setH(false);

		if (((ArmoredVindicatorEntity)entityIn).getItemBySlot(EquipmentSlotType.HEAD).getItem() != ModItems.GOLD_VINDICATOR_HELMET.get() && entityIn.getItemBySlot(EquipmentSlotType.HEAD).getItem() != ModItems.DIAMOND_VINDICATOR_HELMET.get()) {
			c.setScaleX(0);
			c.setScaleY(0);
			c.setScaleZ(0);
		}else {
