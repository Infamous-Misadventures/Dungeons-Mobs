...
public class LeapleafModel<T extends LeapleafEntity> extends SegmentedModel<T> {
	...
	public void setRotationAngles(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch){
		...
		this.rightLeg.rotateAngleX = MathHelper.cos(limbSwing * 0.6662F) * 1.4F * limbSwingAmount * 0.5F;// since the Leapleaf's legs are relatively small, need to reduce the angle change by 50%
		this.leftLeg.rotateAngleX = MathHelper.cos(limbSwing * 0.6662F + (float)Math.PI) * 1.4F * limbSwingAmount * 0.5F;// since the Leapleaf's legs are relatively small, need to reduce the angle change by 50%
