...
public class ParticleGenerator {
	...
	@Nullable
	public static Particle addParticleInternal(ParticleOptions particleData, boolean guarantee, double x, double y,
	...
	@Nullable
	private static Particle addParticleInternal(ParticleOptions particleData, boolean guarantee, boolean minimize,
		...
		if (activerenderinfo.isInitialized() && MINECRAFT.particleEngine != null) {
			...
			if (guarantee) {
				...
				return particlestatus == ParticleStatus.MINIMAL ? null
						: MINECRAFT.particleEngine.createParticle(particleData, x, y, z, xSpeed, ySpeed, zSpeed);
			}
		} else {
			return null;
		}
	...
	public static void generatePotionImpact(Level level, Potion potion, ItemStack itemStack, BlockPos blockPos,
		...
		for (int particleIndex = 0; particleIndex < 8; ++particleIndex) {
			level.addParticle(new ItemParticleOption(ParticleTypes.ITEM, itemStack), bottomCenterOf.x, bottomCenterOf.y,
					bottomCenterOf.z, level.random.nextGaussian() * 0.15D, level.random.nextDouble() * 0.2D,
		...
		for (int internalParticleCount = 0; internalParticleCount < 100; ++internalParticleCount) {
			...
			Particle particleInternal = addParticleInternal(particleData, particleData.getType().getOverrideLimiter(),
					bottomCenterOf.x + d29 * 0.1D, bottomCenterOf.y + 0.3D, bottomCenterOf.z + d7 * 0.1D, d29, d5, d7);
