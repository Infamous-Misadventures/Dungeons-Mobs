...
public class AvoidBaseEntityGoal<T extends Entity> extends Goal {
	...
	public AvoidBaseEntityGoal(PathfinderMob entityIn, Class<T> avoidClass, float distance, double farSpeedIn,
			double nearSpeedIn) {
	...
	public boolean canUse() {
		this.avoidTarget = this.getNearestEntity(this.classToAvoid, this.hostCreature, this.hostCreature.getX(),
				this.hostCreature.getY(), this.hostCreature.getZ(),
				this.hostCreature.getBoundingBox().inflate(this.avoidDistance, 3.0D, this.avoidDistance));
		if (this.avoidTarget == null) {
			...
			if (vector3d == null) {
				return false;
			} else if (this.avoidTarget.distanceToSqr(vector3d.x, vector3d.y, vector3d.z) < this.avoidTarget
					.distanceToSqr(this.hostCreature)) {
				return false;
			} else {
	...
	@Nullable
	private <T extends Entity> T getNearestEntity(Class<? extends T> entityClass, LivingEntity livingEntity,
			double xPos, double yPos, double zPos, AABB axisAlignedBB) {
		return this.getClosestEntity(livingEntity.level.getEntitiesOfClass(entityClass, axisAlignedBB, ALIVE),
				livingEntity, xPos, yPos, zPos);
	}

	@Nullable
	private <T extends Entity> T getClosestEntity(List<? extends T> nearbyEntities, LivingEntity livingEntity,
