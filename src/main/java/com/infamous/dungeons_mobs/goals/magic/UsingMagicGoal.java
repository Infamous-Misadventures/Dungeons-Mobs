...
public class UsingMagicGoal<T extends Mob & IMagicUser> extends Goal {
	...
	public void tick() {
		if (this.hostMobEntity.getTarget() != null) {
			this.hostMobEntity.getLookControl().setLookAt(this.hostMobEntity.getTarget(),
					(float) this.hostMobEntity.getMaxHeadYRot(), (float) this.hostMobEntity.getMaxHeadXRot());
		}
