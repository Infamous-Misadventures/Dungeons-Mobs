...
public class ShieldAndMeleeAttackGoal<T extends PathfinderMob & IShieldUser> extends MeleeAttackGoal {
	...
	public ShieldAndMeleeAttackGoal(T creatureEntity, double speedTowardsTarget, boolean useLongMemory,
			double closeQuartersRange, int maxCloseQuartersShieldUseTime, int attackWindowTime) {
	...
	private boolean hasShieldInOffhand() {
		return this.hostCreature.getOffhandItem().canPerformAction(net.minecraftforge.common.ToolActions.SHIELD_BLOCK); // using
																														// the
																														// Forge
																														// ItemStack-sensitive
																														// version
	...
	public void tick() {
		...
		if (hasShield // check if we have a shield - if not, we must default to melee attack AI
				...
				&& !dontShieldAgainst) { // check if the target is a villager - if so, default to melee attack AI since
											// it cannot fight back
			double hostDistanceSq = this.hostCreature.distanceToSqr(attackTarget.getX(), attackTarget.getY(),
					attackTarget.getZ());
