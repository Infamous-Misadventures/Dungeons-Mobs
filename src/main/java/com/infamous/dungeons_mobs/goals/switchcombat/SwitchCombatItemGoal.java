...
public class SwitchCombatItemGoal extends Goal {
	...
	public boolean canUse() {
		...
		if (target == null) {
			...
			// check if we are close to the target and have a ranged item in mainhand and do
			// not have one in offhand,
			// or if we are far from the target and we do not have a ranged item in our
			// mainhand but do have one in our offhand
			return ((this.hostMob.distanceTo(this.target) < minDistance && hasRangedItemInMainhand()
					&& !hasRangedItemInOffhand())
					|| (this.hostMob.distanceTo(this.target) > maxDistance && !hasRangedItemInMainhand())
							&& hasRangedItemInOffhand())
	 ...
	 */
	public void tick() {
		if (this.hostMob.distanceTo(this.target) < minDistance && hasRangedItemInMainhand()
				&& !hasRangedItemInOffhand()) {
			swapWeapons();
		} else if (this.hostMob.distanceTo(this.target) > maxDistance && !hasRangedItemInMainhand()
				&& hasRangedItemInOffhand()) {
			swapWeapons();
		}
	}
