...
public class ThrowAndMeleeAttackGoal<T extends PathfinderMob & RangedAttackMob> extends MeleeAttackGoal {
	...
	public ThrowAndMeleeAttackGoal(T rangedAttackMob, double speedAmplifier, int attackInterval, float maxDistance,
	...
	public void tick() {
		...
		if (hasThrowableItemInMainhand() && attackTarget != null) {
			float hostDistanceSq = (float) this.hostCreature.distanceToSqr(attackTarget.getX(), attackTarget.getY(),
					attackTarget.getZ());
			...
			if (--this.rangedAttackTime == 0) {
				...
				if (this.hasThrowableItemInMainhand())
					this.hostCreature.swing(InteractionHand.MAIN_HAND);

				this.hostCreature.performRangedAttack(attackTarget, clampedDistanceOverAttackRadius);
				this.rangedAttackTime = Mth
						.floor(distanceOverAttackRadius * (float) (this.maxRangedAttackTime - this.attackIntervalMin)
