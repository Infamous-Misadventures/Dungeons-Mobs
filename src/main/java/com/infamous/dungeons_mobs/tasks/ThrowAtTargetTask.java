...
public class ThrowAtTargetTask<E extends Mob> extends Behavior<E> {
	...
	public ThrowAtTargetTask(Predicate<ItemStack> throwItemPredicate, BiConsumer<E, LivingEntity> performRangedAttack) {
		super(ImmutableMap.of(MemoryModuleType.LOOK_TARGET, MemoryStatus.REGISTERED, MemoryModuleType.ATTACK_TARGET,
				MemoryStatus.VALUE_PRESENT), 1200);
	...
	protected boolean checkExtraStartConditions(ServerLevel serverWorld, E thrower) {
		...
		return thrower.isHolding(this.throwItemPredicate) && BehaviorUtils.canSee(thrower, attackTarget)
				&& BehaviorUtils.isWithinAttackRange(thrower, attackTarget, 0);
	}

	protected boolean canStillUse(ServerLevel serverWorld, E thrower, long gameTime) {
		return thrower.getBrain().hasMemoryValue(MemoryModuleType.ATTACK_TARGET)
				&& this.checkExtraStartConditions(serverWorld, thrower);
	}
