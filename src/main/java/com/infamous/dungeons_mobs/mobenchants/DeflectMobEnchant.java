...
public class DeflectMobEnchant extends MobEnchant {
	...
	public static void onDeflectImpact(ProjectileImpactEvent event) {
		...
		if (entity instanceof Projectile) {
			...
			if (!projectileHitLivingEntity(rayTraceResult))
				return;
	...
	private static void deflectProjectile(Projectile projectile, LivingEntity victim) {
		if (projectile instanceof ShulkerBullet) {
			 ...
			 * DamagingProjectileEntity damagingProjectileEntity =
			 * (DamagingProjectileEntity) projectile; double x0 =
			 * projectile.getOwner().getX() - victim.getX(); double y0 =
			 * projectile.getOwner().getY(0.5) - projectile.getY(); double z0 =
			 * projectile.getOwner().getZ() - victim.getZ(); float f =
			 * MathHelper.sqrt(MathHelper.sqrt(x0)) * 0.5F; double x1 = x0 +
			 * victim.getRandom().nextGaussian() * f; double y1 = y0; double z1 = z0 +
			 * victim.getRandom().nextGaussian() * f; double speed =
			 * (double)MathHelper.sqrt(x1 * x1 + y1 * y1 + z1 * z1); if (speed != 0.0D) {
			...
			double speed = (double) Mth.sqrt((float) (deltaMovement.x * deltaMovement.x
					+ deltaMovement.y * deltaMovement.y + deltaMovement.z * deltaMovement.z));
			...
			projectile.shoot(d0, d1 + d3 * (double) 0.2F, d2, (float) speed,
